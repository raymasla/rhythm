...
let currentNoteSpeed = 2.0; 
let currentAudioSource = null; // 控制 MP3 播放的變數
...
function audioScheduler() {
    while (nextBeatTime < audioCtx.currentTime + 0.1) {
        playPianoSound(600, nextBeatTime); nextBeatTime += beatInterval;
    }
    audioTimerId = setTimeout(audioScheduler, 25.0);
}

// 鋼琴聲合成器
function playPianoSound(frequency, time) {
    if(!audioCtx) return;
...
function initCarousel() {
    ...
    songs.forEach((song, i) => {
        ...
        div.onclick = () => { currentSongIndex = i; updateCarouselPositions(); if (i === 0 || i === 1) openModal(); else alert("尚未解鎖！"); };
...
window.addEventListener('keydown', (e) => {
    if (!document.getElementById('step-song-select').classList.contains('hidden') && document.getElementById('difficulty-modal').classList.contains('hidden')) {
        ...
        else if (e.key === 'Enter') { if (currentSongIndex === 0 || currentSongIndex === 1) openModal(); else alert("尚未解鎖！"); }
    }
});
let touchStartY = 0;
...
function prepareGame(difficulty) {
    ...
    let rulesText = "";
    if (difficulty === 'extreme-easy') rulesText = "【極度簡單】(4軌)<br>點擊螢幕任意處或鍵盤任意鍵。";
    else if (difficulty === 'easy') rulesText = "【簡單模式】(4軌)<br>螢幕分左右兩側。<br>電腦版：左 F / 右 J。";
    else if (difficulty === 'normal') rulesText = "【普通模式】(4軌)<br>電腦版：D, F, J, K。";
    else if (difficulty === 'hard') rulesText = "【極度困難】(6軌)<br>手機版：手指滑過軌道。<br>電腦版：普通鍵為 S, D, F, J, K, L<br>滑動鍵為 E(軌道2), I(軌道5)。";
...
function generateLanes() {
    ...
    for (let i = 0; i < laneCount; i++) {
        let laneDiv = document.createElement('div'); laneDiv.className = 'lane';
        let padDiv = document.createElement('div'); padDiv.className = 'hit-pad'; padDiv.id = `pad-${i}`;
        
        // Hard 模式下的第 1 軌 (軌道2, D/E) 和第 4 軌 (軌道5, K/I) 套用粉色
        if (currentDifficulty === 'hard' && (i === 1 || i === 4)) padDiv.classList.add('slide-pad');
...
function showJudgement(type) {
    const jText = document.getElementById('judgement-text'); const cText = document.getElementById('combo-text');
...
function hitLane(laneIndex, isSlideInput) {
    ...
    if (pad) {
        ...
        if(!isSlideInput) playPianoSound(frequencies[laneIndex % frequencies.length], audioCtx.currentTime);
    ...
    if (validNotes.length > 0) {
        ...
        if (!isSlideInput && timeDiff > 0.18) {
            target.hit = true; stats.C++; showJudgement('C');
...
window.addEventListener('touchmove', (e) => {
    if (!document.getElementById('step-game').classList.contains('hidden')) {
        e.preventDefault(); 
        if (currentDifficulty === 'hard') {
            let section = window.innerWidth / 6;
            for(let i=0; i<e.changedTouches.length; i++) {
                let lane = Math.floor(e.changedTouches[i].clientX / section);
...
async function loadAndPlaySong() {
    try {
        ...
        // 讀音樂
        try {
            const audioRes = await fetch(`songs/${songId}/audio.mp3`);
            if (audioRes.ok) {
                ...
                currentAudioSource.start(gameStartTime);
            } else {
                console.warn("未找到 MP3 檔案，僅啟動譜面引擎");
                gameStartTime = audioCtx.currentTime + 2.0; 
            }
        } catch(e) {
            console.warn("無法加載音頻，僅啟動譜面引擎", e);
            gameStartTime = audioCtx.currentTime + 2.0; 
        }
        ...
        console.error(error);
        alert("讀取失敗！請確認 Live Server 是否開啟與檔案路徑。");
    }
}
...
function showScoreboard() {
    ...
    let modeText = "";
    if (currentDifficulty === 'extreme-easy') modeText = "MODE: EXTREME EASY (極度簡單 4軌)";
    else if (currentDifficulty === 'easy') modeText = "MODE: EASY (簡單 4軌)";
    else if (currentDifficulty === 'normal') modeText = "MODE: NORMAL (普通 4軌)";
    else if (currentDifficulty === 'hard') modeText = "MODE: HARD (極度困難 6軌)";
